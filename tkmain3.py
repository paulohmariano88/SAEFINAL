...
class VisionApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Sistema de Visão Computacional SAETOWER")
        ...
        title = tk.Label(self.control_frame, text="Configuração de Áreas", font=("Arial", 14, "bold"), fg="white", bg="#3B3B3B")
        title.pack(pady=10)

        # Criar seções de controle
        self.create_controls("ÁREA CRÍTICA", "#00FF00", self.update_critical_width, self.update_critical_height)
        self.create_controls("ÁREA PERIGOSA", "#0000FF", self.update_danger_width, self.update_danger_height)
    ...
    def create_controls(self, title, color, update_width_func, update_height_func):
        ...
        tk.Label(frame, text="Largura:", bg="#3B3B3B", fg="white").pack(anchor="w", padx=5)
        width_slider = ttk.Scale(frame, from_=50, to=500, orient="horizontal", command=update_width_func)
        width_slider.set(150 if title == "ÁREA CRÍTICA" else 250)
        width_slider.pack(fill="x", padx=5)

        tk.Label(frame, text="Altura:", bg="#3B3B3B", fg="white").pack(anchor="w", padx=5)
        height_slider = ttk.Scale(frame, from_=50, to=500, orient="horizontal", command=update_height_func)
        height_slider.set(200 if title == "ÁREA CRÍTICA" else 300)
    ...
    def draw_areas(self, frame, is_danger, is_critical):
        ...
        if is_critical:
            cv2.putText(frame, "STOP", (center_x - 50, center_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 5)
